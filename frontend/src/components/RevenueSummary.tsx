...
/**
 * Formats a decimal amount string (e.g. "1000.000") as "1,000.00" using only
 * integer/BigInt math. Avoids parseFloat/Number(), which reintroduce binary
 * floating-point rounding error on monetary values.
 */
function formatMoney(rawTotal: string): string {
    const trimmed = (rawTotal ?? '0').trim();
    const negative = trimmed.startsWith('-');
    const unsigned = negative ? trimmed.slice(1) : trimmed;
    const [wholeRaw, fracRaw = ''] = unsigned.split('.');
    const whole = wholeRaw || '0';

    // Pad to (at least) 3 fractional digits so we always have a digit to round on.
    const fracPadded = (fracRaw + '000').slice(0, 3);
    let cents = BigInt(whole + fracPadded.slice(0, 2));
    if (Number(fracPadded[2]) >= 5) {
        cents += 1n;
    }

    const centsStr = cents.toString().padStart(3, '0');
    const wholePart = centsStr.slice(0, -2);
    const centPart = centsStr.slice(-2);
    const wholeWithSeparators = wholePart.replace(/\B(?=(\d{3})+(?!\d))/g, ',');

    return `${negative ? '-' : ''}${wholeWithSeparators}.${centPart}`;
}
